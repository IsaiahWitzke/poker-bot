...
Add 2 numbers together. Will make 2 inputs, and one output. No hidden layers. This entire neural network can be represented by 1 linear expression: <img src="https://latex.codecogs.com/gif.latex?h_{\theta}(x) = W_{\theta}x + b_{\theta}" />, where <img src="https://latex.codecogs.com/gif.latex?W_{\theta} \in \mathbb{R}^{2x1}" /> represents all the weights that make up the network, and <img src="https://latex.codecogs.com/gif.latex?b_{\theta} \in \mathbb{R}^{2}" /> represent all the biases. As can be seen, no non-linear ReLU or sigmoid function are found here yet, so this just acts as a simple linear model. This should be appropriate since all we are trying to do is predict the sum of 2 input variables - in this case, these 2 inputs are represented by <img src="https://latex.codecogs.com/gif.latex?x \in \mathbb{R}^{2}" />).
